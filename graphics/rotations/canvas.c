...
/* The current free scene mode */
int free_scene_mode = 0;


/* Globals for animation management */
static bool animate = false;
static int const FPS = 30;
GLfloat currentAngleOfRotation = 0.0;
float dir = 0.01f;

/* Extra in display mode 7 for mathematical shading */
...
/* Cone Triangle Calculation Parameteres */
GLfloat radius = 1.0f;
GLfloat height = 1.0f;
GLint sides = 8;
...
/*
 * Timer to deal with animation
 */
void timer(int v) {
    
    //Computes the rotations on square
    if(free_scene_mode == FREE_SQUARE_ROT) {
        if (animate) {
            currentAngleOfRotation += dir;
            if (currentAngleOfRotation > 2.0 && currentAngleOfRotation < 358.0) {
                currentAngleOfRotation = 358.0;
            }
    ...
    //Changes the camera angle rapidly to make it appear as the stars are
    //Shooting past and the meteor has small perturbations.
    if(free_scene_mode == FREE_METEOR_SPACE) {
        if(animate) {
            u += 1;
            gluLookAt(0, 0, 0.05*cos(1-u), 0.01*cos(u), -0.01, -0.01, 0, 0.01*cos(u), 0);
            glutPostRedisplay();
        }
    }
    
    //Tells the timer how many times to reset
